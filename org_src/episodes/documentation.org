...
Our =chemlib= package now has tests, a changelog, CI, and a release pipeline.
...
arguments does =center_of_mass= expect? What does =calc_distance= return?
...
#+begin_challenge
{{{mdheader(## Challenge: Document the Fortran Wrapper)}}}

If you completed the Compiled Extensions episode, =chemlib= also exposes
=calc_distance=. Write a NumPy-style docstring for a Python wrapper function
that calls the Fortran kernel.
...
If you completed the CI episode, you already understand workflow files. The

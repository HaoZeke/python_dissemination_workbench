...
#+BEGIN_SRC dot :file ../../episodes/fig/uv-pep723-flow.png :cmdline -Kdot -Tpng -Gdpi=150 :cache yes :exports none
digraph UVFlow {
    rankdir=LR;
    ...
    script [label="script.py\n(Metadata Header)", shape=note, fillcolor="#fff9c4"];

    // The Tool
    uv [label="uv run", shape=box, style="rounded,filled", fillcolor="#29b6f6", fontcolor="white"];
    ...
    subgraph cluster_env {
        ...
        venv [label="1. Create/Cache Venv", shape=box, fillcolor="#e1f5fe"];
        install [label="2. Install 'numpy'", shape=box, fillcolor="#b3e5fc"];
        run [label="3. Execute Code", shape=component, fillcolor="#81d4fa"];
    ...
    // Edges
    user -> uv [label="Runs"];
    script -> uv [label="Input"];
    uv -> venv [label="Parses Header"];
}
#+END_SRC

#+RESULTS[b3a143a9d3ffb6b10bccf134e617ef4318364ed7]:

@@markdown:![Flowchart showing uv taking a script with metadata, creating a temporary environment, installing dependencies, and executing the code](fig/uv-pep723-flow.png)@@
...
#+BEGIN_SRC dot :file ../../episodes/fig/pixi-uv-stack.png :cmdline -Kdot -Tpng -Gdpi=150 :cache yes :exports none
digraph PixiStack {
    rankdir=TB;
    ...
    // Outer Layer: Pixi
    subgraph cluster_pixi {
        label="Layer 1: Pixi (System & Binaries)";
        style=filled;
        fillcolor="#fff3e0"; // Orange tint
        color="#ffcc80";

        pixi_bin [label="Binaries\n(eonclient)", fillcolor="#ffe0b2"];
        
        // Inner Layer: UV
        subgraph cluster_uv {
            label="Layer 2: UV (Python Packages)";
            style=filled;
            fillcolor="#e1f5fe"; // Blue tint
            color="#81d4fa";

            uv_libs [label="Python Libraries\n(numpy, ase, metatomic)", fillcolor="#b3e5fc"];
            
            // The Script
            script [label="eon_min.py", shape=note, fillcolor="#ffffff", penwidth=2];
        }
    }

    // Relationships
    pixi_bin -> script [label="Shell Access\n(subprocess.run)", style=dashed, lhead=cluster_uv];
    uv_libs -> script [label="Import Access\n(import numpy)"];
}
#+END_SRC

#+RESULTS[aa278f3c87957881f9fbc5dc632c76d1fc12d2ee]:

@@markdown:![Diagram showing the nested layers: Pixi providing system binaries like eonclient, wrapping UV which provides Python libraries like numpy, both supporting the script](fig/pixi-uv-stack.png)@@
...
Create a script named =ase_min.py= that performs the exact same minimization on
=pos.con=, but uses the [[https://ase-lib.org/][atomic simulation environment (ASE)]] built-in =LBFGS=
optimizer instead of eOn.
